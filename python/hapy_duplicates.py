...
#from science_firstlook import add_science_columns, add_qc_flags
...
class duplicates():
    def __init__(self,hatab_filename,googlesheet=None,vf=None):
        """
        PARAMS:
        ...
        * vf = main vf tables
        ...
        tel_int = (1)*(self.fulltab['TELESCOPE'] == 'INT') + \
          2*(self.fulltab['TELESCOPE'] == 'BOK') + \
          4*(self.fulltab['TELESCOPE'] == 'HDI') + \
          8*(self.fulltab['TELESCOPE'] == 'MOS')
        ...
        self.v = vf
        self.htab = self.fulltab

    def get_augmented_table(self):
        #self.htab = add_science_columns(self.htab)
        #self.htab = add_qc_flags(self.htab)

        self.htab = prepare_analysis_table(self.htab)
    def get_sample(self):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hapytable', dest = 'hapytab', default = 'merged_results.fits', help = 'merged table of hapy run_analysis')    
    ...
    hafilename = args.hapytab
    d = duplicates(hafilename)
    #d.get_sample()
    d.get_duplicates()
    #d.get_smorph_flag()
